...
const loadProfile = async (id) => {
    ...
    if (data.stateCad == true) {
        document.querySelector('.delete').innerHTML = `<button class="changePass" onclick="deletUser(${id})">Deletar Usuário</button>`
    }
    else{
        document.querySelector('.delete').innerHTML = `<button class="changePass" onclick="reactivate(${id})">Reativar Usuário</button>`
    }
}
...
const reactivate = async (index) => {
    ...
    if (confirmacao == true) {
        const responseSM = await fetch(`http://localhost:1313/perfilreative/${index}?`);
        const data = await responseSM.json();
